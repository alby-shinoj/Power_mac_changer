...
def read_mac_addresses(file_path):
    ...
    try:
        with open(file_path, 'r') as f:
            mac_addresses = [line.strip() for line in f if validate_mac(line.strip())]
        if not mac_addresses:
            logging.error("No valid MAC addresses found in the file.")
            sys.exit(1)
        return mac_addresses
    except Exception as e:
        logging.error(f"Error reading MAC addresses from file: {e}")
        sys.exit(1)
...
def main():
    ...
    mac_file = "mac_addresses.txt"  # File containing MAC addresses
    mac_addresses = read_mac_addresses(mac_file)

    # Display MAC addresses for selection
    print("Available MAC addresses:")
    for i, mac in enumerate(mac_addresses):
        print(f"{i + 1}. {mac}")
    choice = int(input("Select the MAC address to use (1, 2, ...): ")) - 1
    new_mac = mac_addresses[choice]
